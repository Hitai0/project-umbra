...
export class WorldRoom extends Room<WorldState> {
  maxClients = 100;
  private attackCooldowns = new Map<string, AttackCooldown>();
  private monsterLastMove = new Map<string, number>();
  ...
  private setupMessageHandlers() {
    ...
    this.onMessage(NetworkAction.MOVE_TO, (client, data: MoveToPayload) => {
    ...
    this.onMessage(NetworkAction.ATTACK_TARGET, (client, data: AttackTargetPayload) => {
      const player = this.state.players.get(client.sessionId);
      if (!player) return;
    ...
    // 3. Chat Message
    this.onMessage(NetworkAction.CHAT_MESSAGE, (client, data: ChatMessagePayload) => {
      const player = this.state.players.get(client.sessionId);
  ...
  private spawnInitialMonsters() {
    const initialSpawns: Array<{ type: MonsterType; count: number }> = [
  ...
  private update(deltaTime: number) {
    ...
    this.state.monsters.forEach((monster) => {
      ...
      const config = MONSTER_CONFIGS[monster.monsterType as MonsterType] || MONSTER_CONFIGS.Poring;
